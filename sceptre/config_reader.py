...
class ConfigReader(object):
    ...
    def _recursive_read(self, directory_path, filename):
        ...
        if not directory_path:
            return self._read(directory_path, filename) or {}

        # Recurse into higher level directory
    ...
    def _read(self, directory_path, basename):
        ...
        :rtype: dict
        """
        directory_path = path.join(self.config_folder, directory_path)
        if path.isfile(path.join(directory_path, basename)):
