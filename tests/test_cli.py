...
class TestCli(object):
    ...
    def setup_method(self, test_method):
        self.patcher_get_env = patch("sceptre.cli.get_env")
        self.patcher_get_stack = patch("sceptre.cli.get_stack")
        self.patcher_getcwd = patch("sceptre.cli.os.getcwd")

        self.mock_get_env = self.patcher_get_env.start()
        ...
        self.mock_get_stack.return_value = MagicMock(spec=Stack)
        self.mock_get_env.return_value = MagicMock(spec=Environment)

        self.runner = CliRunner()

    def teardown_method(self, test_method):
        self.patcher_get_env.stop()
        self.patcher_get_stack.stop()
    ...
    def test_validate_template(self):
        ...
        self.mock_get_stack.assert_called_with(sentinel.cwd, "dev", "vpc")
        self.mock_get_stack.return_value.template.validate.assert_called_with()

    def test_generate_template(self):
        self.mock_get_stack.return_value.template.body = "body"
    ...
    def test_lock_stack(self):
        ...
        self.mock_get_stack.assert_called_with(sentinel.cwd, "dev", "vpc")
        self.mock_get_stack.return_value.lock.assert_called_with()

    def test_unlock_stack(self):
        self.runner.invoke(cli, ["unlock-stack", "dev", "vpc"])
        self.mock_get_stack.assert_called_with(sentinel.cwd, "dev", "vpc")
        self.mock_get_stack.return_value.unlock.assert_called_with()

    def test_describe_env_resources(self):
        self.mock_get_env.return_value.describe_resources.return_value = {
            "stack-name-1": {
                "StackResources": [
                    {
        ...
        self.mock_get_env.assert_called_with(sentinel.cwd, "dev")
        self.mock_get_env.return_value.describe_resources\
            .assert_called_with()
        # Assert that there is output
        assert result.output

    def test_describe_stack_resources(self):
        self.mock_get_stack.return_value.describe_resources.return_value = {
                "StackResources": [
                    {
                        "LogicalResourceId": "logical-resource-id",
                        "PhysicalResourceId": "physical-resource-id"
        ...
        self.mock_get_stack.assert_called_with(sentinel.cwd, "dev", "vpc")
        self.mock_get_stack.return_value.describe_resources\
            .assert_called_with()
        # Assert that there is output.
        assert result.output
    ...
    def test_create_stack(self):
        ...
        self.mock_get_stack.assert_called_with(sentinel.cwd, "dev", "vpc")
        self.mock_get_stack.return_value.create.assert_called_with()

    def test_delete_stack(self):
        self.runner.invoke(cli, ["delete-stack", "dev", "vpc"])
        self.mock_get_stack.assert_called_with(sentinel.cwd, "dev", "vpc")
        self.mock_get_stack.return_value.delete.assert_called_with()

    def test_update_stack(self):
        self.runner.invoke(cli, ["update-stack", "dev", "vpc"])
        self.mock_get_stack.assert_called_with(sentinel.cwd, "dev", "vpc")
        self.mock_get_stack.return_value.update.assert_called_with()

    def test_launch_stack(self):
        self.runner.invoke(cli, ["launch-stack", "dev", "vpc"])
        self.mock_get_stack.assert_called_with(sentinel.cwd, "dev", "vpc")
        self.mock_get_stack.return_value.launch.assert_called_with()

    def test_launch_env(self):
        self.runner.invoke(cli, ["launch-env", "dev"])
        self.mock_get_env.assert_called_with(sentinel.cwd, "dev")
        self.mock_get_env.return_value.launch.assert_called_with()

    def test_launch_env_returns_zero_correctly(self):
        self.mock_get_env.return_value.launch.return_value = dict(
    ...
    def test_delete_env(self):
        self.mock_get_env.return_value.delete.return_value = sentinel.response
        self.runner.invoke(cli, ["delete-env", "dev"])
        self.mock_get_env.assert_called_with(sentinel.cwd, "dev")
        self.mock_get_env.return_value.delete.assert_called_with()

    def test_delete_env_returns_zero_correctly(self):
        self.mock_get_env.return_value.delete.return_value = dict(
            (sentinel.stack_name, StackStatus.COMPLETE) for _ in range(5)
        )
    ...
    def test_continue_update_rollback(self):
        ...
        self.mock_get_stack.assert_called_with(sentinel.cwd, "dev", "vpc")
        self.mock_get_stack.return_value.continue_update_rollback\
            .assert_called_with()
    ...
    def test_create_change_set(self):
        ...
        self.mock_get_stack.assert_called_with(sentinel.cwd, "dev", "vpc")
        self.mock_get_stack.return_value.create_change_set\
    ...
    def test_delete_change_set(self):
        ...
        self.mock_get_stack.assert_called_with(sentinel.cwd, "dev", "vpc")
        self.mock_get_stack.return_value.delete_change_set\
            .assert_called_with("cs1")

    def test_describe_change_set(self):
        self.mock_get_stack.return_value.describe_change_set.return_value = {
        ...
        self.mock_get_stack.assert_called_with(sentinel.cwd, "dev", "vpc")
        self.mock_get_stack.return_value.describe_change_set\
            .assert_called_with("cs1")
    ...
    def test_execute_change_set(self):
        ...
        self.mock_get_stack.assert_called_with(sentinel.cwd, "dev", "vpc")
        self.mock_get_stack.return_value.execute_change_set\
            .assert_called_with("cs1")

    def test_list_change_sets(self):
        self.runner.invoke(cli, ["list-change-sets", "dev", "vpc"])
        self.mock_get_stack.assert_called_with(sentinel.cwd, "dev", "vpc")
        self.mock_get_stack.return_value.list_change_sets.assert_called_with()

    @patch("sceptre.cli.uuid1")
    def test_update_with_change_set_with_input_yes(self, mock_uuid1):
        self.mock_get_stack.return_value.wait_for_cs_completion\
    ...
    def test_describe_stack_outputs(self):
        ...
        self.mock_get_stack.assert_called_with(sentinel.cwd, "dev", "vpc")
        self.mock_get_stack.return_value.describe_outputs.assert_called_with()

    def test_describe_stack_outputs_handles_envvar_flag(self):
        self.mock_get_stack.return_value.describe_outputs\
            .return_value = [
                {
                    "OutputKey": "key",
                    "OutputValue": "value"
    ...
    def test_describe_env(self):
        self.mock_get_env.return_value.describe.return_value = {
            "stack": "status"
        }
    ...
    def test_set_stack_policy_with_file_flag(self):
        ...
        self.mock_get_stack.assert_called_with(sentinel.cwd, "dev", "vpc")
        self.mock_get_stack.return_value.set_policy.assert_called_with(
            policy_file
        )

    def test_get_stack_policy_with_existing_policy(self):
        self.mock_get_stack.return_value.get_policy.return_value = {
    ...
    @patch("sceptre.cli.ConfigReader")
    def test_get_env(self, mock_ConfigReader):
        self.patcher_get_env.stop()
        config_reader = MagicMock(spec=ConfigReader)
        config_reader.construct_environment.return_value = sentinel.environment
        mock_ConfigReader.return_value = config_reader
        response = sceptre.cli.get_env(sentinel.cwd, sentinel.environment_path)
        assert response == sentinel.environment
        self.patcher_get_env.start()
